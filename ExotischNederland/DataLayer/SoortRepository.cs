...
internal class SoortRepository
{
    ...
    public List<Model.Soort> FilterSoortenOorsprong(char filter)
    {
        ...
        string selectQuery = @"SELECT * FROM Soort WHERE Oorsprong = @Oorsprong;";
        using var command = new SqliteCommand(selectQuery, connection);
        command.Parameters.AddWithValue("@Oorsprong", filter);
        using var reader = command.ExecuteReader();
        while (reader.Read())
        {
            int id = reader.GetInt32(0);
            ...
            gefilterdeSoorten.Add(soort);
        }
        return gefilterdeSoorten;
    }
}
